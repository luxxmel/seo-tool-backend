...
public class SupabaseService {
    ...
    private void insertUrl(String url, String httpStatus, String pingStatus, String discoveryStatus) throws Exception {
        ...
        try {
            ...
            System.out.println("SUPABASE INSERT STATUS: " + status);
            System.out.println("SUPABASE INSERT BODY: " + body);

            if (status < 200 || status >= 300) {
                throw new RuntimeException("Insert failed with status " + status + ": " + body);
            }
        } finally {
            if (connection != null) {
                connection.disconnect();
    ...
    private void updateUrl(String url, String httpStatus, String pingStatus, String discoveryStatus, int pingCount) throws Exception {
        ...
        try {
            ...
            System.out.println("SUPABASE UPDATE STATUS: " + status);
            System.out.println("SUPABASE UPDATE BODY: " + body);

            if (status < 200 || status >= 300) {
                throw new RuntimeException("Update failed with status " + status + ": " + body);
            }
        } finally {
            if (connection != null) {
                connection.disconnect();
    ...
    private int getCurrentPingCount(String url) {
        ...
        try {
            ...
            if (status < 200 || status >= 300) {
                System.out.println("SUPABASE COUNT STATUS: " + status);
                System.out.println("SUPABASE COUNT BODY: " + readBody(connection, status));
                return 0;
            }
            ...
            System.out.println("SUPABASE COUNT ERROR: " + e.getMessage());
            return 0;
        } finally {
            if (connection != null) {
                connection.disconnect();
