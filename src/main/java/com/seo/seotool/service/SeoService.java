...
public class SeoService {
     ...
     * - GOOGLE_BLOCK nghĩa là Google chặn request search, không được kết luận URL chưa index.
     */
    public List<Map<String, Object>> processCheckIndex(List<String> urls) {
        ...
        for (String rawUrl : urls) {
            ...
            if (!isValidHttpUrl(url)) {
                ...
                item.put("note", "URL không hợp lệ, không thể kiểm tra index.");
            ...
            if ("GOOGLE_BLOCK".equals(googleIndex.status)) {
                note = "Google chặn request kiểm tra, chưa xác định được index.";
            } else if ("INDEXED".equals(googleIndex.status)) {
                if (checked.noindex) {
                    note = "Đã thấy trên Google, nhưng URL hiện có noindex.";
                } else if (checked.blocked) {
                    note = "Đã thấy trên Google, nhưng server hiện có dấu hiệu chặn request.";
                } else if (!checked.alive) {
                    note = "Đã thấy trên Google, nhưng URL hiện không phản hồi HTTP 2xx/3xx.";
                } else {
                    note = "Đã thấy URL trên Google.";
                }
            } else if ("NOT_INDEXED".equals(googleIndex.status)) {
                if (checked.noindex) {
                    note = "Chưa thấy trên Google. URL hiện có noindex nên khó được index.";
                } else if (!checked.alive) {
                    note = "Chưa thấy trên Google. URL hiện lỗi hoặc không phản hồi tốt.";
                } else if (checked.blocked) {
                    note = "Chưa thấy trên Google. URL có dấu hiệu chặn/captcha/server từ chối.";
                } else if (checked.loginWall) {
                    note = "Chưa thấy trên Google. URL có dấu hiệu login wall.";
                } else {
                    note = "Chưa thấy URL trên Google.";
                }
            } else {
        ...
        try {
            ...
            if (statusCode == 429 || statusCode == 503
                ...
                return new GoogleIndexResult(
                        null,
                        "GOOGLE_BLOCK",
                        "Google chặn request kiểm tra index."
                );
            }
            ...
            if (body.contains("did not match any documents")
                ...
                return new GoogleIndexResult(
                        false,
                        "NOT_INDEXED",
                        "Chưa thấy URL trên Google."
                );
            }

            String cleanTarget = normalizeUrlForCompare(url);
            String cleanBody = normalizeHtmlForUrlCompare(body);

            if (cleanBody.contains(cleanTarget)) {
                return new GoogleIndexResult(
                        true,
                        "INDEXED",
            ...
            String withoutTrailingSlash = cleanTarget.replaceAll("/$", "");

            if (cleanBody.contains(withoutTrailingSlash)) {
                return new GoogleIndexResult(
                        true,
                        "INDEXED",
                        "Đã thấy URL trên Google."
            ...
            return new GoogleIndexResult(
                    null,
                    "ERROR",
                    "Lỗi khi kiểm tra Google index: " + e.getMessage()
            );
        } finally {
            if (connection != null) {
    ...
    private String normalizeHtmlForUrlCompare(String html) {
        if (html == null) {
            return "";
        }
